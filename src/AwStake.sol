    ...
    PausableUpgradeable
{
    ...
    struct StakePool {
        ...
        uint256 totalRewards; //总奖励, 精度1e18
        uint256 totalUsers; //总用户数
        uint256 totalRewardsClaimed; //总奖励领取量
        uint256 lastRewardBlock; //上次奖励区块
        uint256 minStakeAmount; //最小质押量
        uint256 unstakeLockedBlocks; //解质押锁定期, 单位区块, 0表示不锁仓
    }

    struct UserStake {
        uint256 amount; //质押量
        uint256 finishedReward; //已领取奖励, 精度1e18
        uint256 pendingReward; //待领取奖励, 精度1e18
        UnstakeRequest[] unstakeRequests; //解质押请求列表
    }

    struct UnstakeRequest {
        uint256 amount; //解质押量
        uint256 unlockBlock; //解质押解锁区块
    }

    IERC20 public rewardToken; //奖励token
    uint256 public rewardStartBlock; //奖励开始区块
    uint256 public rewardEndBlock; //奖励结束区块
    uint256 public rewardPerBlock; //每区块奖励, 精度1e18
    uint256 public totalStakeWeight; //总质押权重
    ...
    mapping(uint256 stakePoolId => mapping(address user => UserStake userStake))
        public userStakes; //用户质押量
     ...
     * @param weight 质押权重
     */
    event StakePoolWeightUpdated(uint256 stakePoolId, uint256 weight);
     ...
     * @param minStakeAmount 最小质押量
     */
    event StakePoolMinStakeAmountUpdated(
        uint256 indexed stakePoolId,
        uint256 minStakeAmount
    );
     ...
     * @param unstakeLockedBlocks 解质押锁定期, 单位区块, 0表示不锁仓
     */
    event StakePoolUnstakeLockedBlocksUpdated(
        uint256 indexed stakePoolId,
        uint256 unstakeLockedBlocks
    );
     ...
     */
    modifier effectiveStakePool(uint256 _stakePoolId) {
    ...
    function setRewardToken(IERC20 _rewardToken) public onlyRole(ADMIN_ROLE) {
        require(
            ...
            "rewardToken must be a valid address"
        );
     ...
     * @param _rewardStartBlock 奖励开始区块
     */
    function setRewardStartBlock(
        uint256 _rewardStartBlock
    ) public onlyRole(ADMIN_ROLE) {
        require(
            _rewardStartBlock > block.number &&
                _rewardStartBlock < rewardEndBlock,
    ...
    function addStakePool(
        ...
        require(
            ...
            "unstakeLockedBlocks must be greater than 0"
        );

        // 设置初始的奖励区块从哪里开始计算
        uint256 _lastRewardBlock = block.number > rewardStartBlock
            ? block.number
            : rewardStartBlock;
    ...
    function updateStakePoolWeight(
        ...
        uint256 _weight
    ) external virtual onlyRole(ADMIN_ROLE) effectiveStakePool(_stakePoolId) {
        require(_weight > 0, "weight must be greater than 0");
        // 更新总质押权重, 先减去旧的权重, 再加上新的权重
        totalStakeWeight =
            totalStakeWeight -
            stakePools[_stakePoolId].weight +
            _weight;
        // 更新质押池权重
    ...
    function updateStakePoolUnstakeLockedBlocks(
        ...
        emit StakePoolUnstakeLockedBlocksUpdated(
            _stakePoolId,
            _unstakeLockedBlocks
        );
    }
     ...
     * @return 区块倍数
     */
    function _getMultiplierByBlockRange(
        uint256 _fromBlock,
        uint256 _toBlock
        ...
        (bool success, uint256 multiplier) = (_toBlock - _fromBlock).tryMul(
            rewardPerBlock
        );
        if (!success) {
     ...
     * @return 用户质押量
     */
    function _getUserStakeInfo(
        uint256 _stakePoolId,
        address _user
    ) internal view returns (UserStake memory) {
    ...
    function getUserStakeAmount(
        uint256 _stakePoolId
    ) external view effectiveStakePool(_stakePoolId) returns (uint256) {
    ...
    function getPendingRewardByBlock(
        ...
        address _user,
        uint256 _block
    ) public view effectiveStakePool(_stakePoolId) returns (uint256) {
        UserStake memory userStake = _getUserStakeInfo(_stakePoolId, _user);
        StakePool memory stakePool = stakePools[_stakePoolId];

        // 获取质押池的总奖励和总质押量
        uint256 stakePoolTotalRewards = stakePool.totalRewards;
        uint256 stakePoolTotalStaked = stakePool.totalStaked;

        if (_block > stakePool.lastRewardBlock && stakePoolTotalStaked > 0) {
            uint256 multiplier = _getMultiplierByBlockRange(
                stakePool.lastRewardBlock,
                _block
            );

            uint256 stakePoolRewardWithWeight = _calculateStakePoolRewardWithWeight(
                    stakePool,
                    multiplier
                );

            stakePoolTotalRewards +=
                (stakePoolRewardWithWeight * 1e18) /
                stakePoolTotalStaked;
        }

        // 计算用户待领取奖励, 公式: (用户质押量 * 质押池总奖励) / 1e18 - 已领取奖励 + 待领取奖励
        return
            ((userStake.amount * stakePoolTotalRewards) / 1e18) -
            userStake.finishedReward +
            userStake.pendingReward;
    }
    ...
    function _calculateStakePoolRewardWithWeight(
        ...
        // 计算质押池奖励, 包含权重
        (bool success, uint256 stakePoolRewardWithWeight) = multiplier.tryMul(
            stakePool.weight
        );
        ...
        (success, stakePoolRewardWithWeight) = stakePoolRewardWithWeight.tryDiv(
            totalStakeWeight
        );
     ...
     * @param _stakePoolId 质押池ID
     */
    function updateStakePoolReward(
        uint256 _stakePoolId
    ) public virtual onlyRole(ADMIN_ROLE) effectiveStakePool(_stakePoolId) {
        StakePool storage stakePool = stakePools[_stakePoolId];
        // 如果区块小于上次奖励区块, 或者总质押量为0, 则不更新奖励
        if (
            block.number <= stakePool.lastRewardBlock ||
            stakePool.totalStaked <= 0
        ) {
            return;
        }
        ...
        (bool success, uint256 addedPerShare) = (stakePoolRewardWithWeight)
            .tryMul(1e18);
        if (!success) {
        ...
        (success, stakePool.totalRewards) = stakePool.totalRewards.tryAdd(
            addedReward
        );
        if (!success) {
    ...
    function _deposit(uint256 _stakePoolId, uint256 _amount) internal {
        if (_amount <= 0) {
    ...
    function _addUserStake(
        ...
        // 计算用户已领取奖励
        uint256 finishedReward = _calculateFinishedReward(
            _stakePool,
            _userStake
        );
    ...
    function _calculateFinishedReward(
        ...
        bool success;
        uint256 newFinishedReward;
        (success, newFinishedReward) = _userStake.amount.tryMul(
            _stakePool.totalRewards
        );
        if (!success) {
    ...
    function _subUserStake(
        StakePool storage _stakePool,
        UserStake storage _userStake,
        uint256 _amount
    ) internal {
        // 更新用户质押量
        (bool success, uint256 newAmount) = _userStake.amount.trySub(_amount);
        if (!success) {
     ...
     * @param _userStake 用户质押
     */
    function _updateUserPendingReward(
        StakePool storage _stakePool,
        UserStake storage _userStake
    ) internal {
        uint256 pendingReward = _calculateUserPendingReward(
            _stakePool,
            _userStake
        );
    ...
    function _calculateUserPendingReward(
        ...
        (bool success, uint256 pendingReward) = _userStake.amount.tryMul(
            _stakePool.totalRewards
        );
        if (!success) {
            revert("userStake.amount * totalRewards overflow");
        ...
        (success, pendingReward) = pendingReward.trySub(
            _userStake.finishedReward
        );
        if (!success) {
            revert(
                "userStake.amount * totalRewards / 1e18 - finishedReward overflow"
            );
        }
        if (pendingReward > 0) {
            (success, pendingReward) = pendingReward.tryAdd(
                _userStake.pendingReward
            );
            if (!success) {
    ...
    function deposit(
        uint256 _stakePoolId,
        uint256 _amount
    ) external whenNotPaused effectiveStakePool(_stakePoolId) {
        if (_stakePoolId == 0) {
        ...
        if (_amount > 0) {
            ...
            IERC20(stakePool.token).safeTransferFrom(
                msg.sender,
                address(this),
                _amount
            );
    ...
    function unstake(
        ...
        // 更新总质押量
        (bool success, uint256 newTotalStaked) = stakePool.totalStaked.trySub(
            _amount
        );
        if (!success) {
            revert("totalStaked - amount overflow");
        ...
        // 更新用户已结算奖励
        uint256 finishedReward = _calculateFinishedReward(stakePool, userStake);
     ...
     * @param _amount 转账金额
     */
    function _safeRewardTokenTransfer(address _to, uint256 _amount) internal {
    ...
    function withdraw(
        ...
        // 计算用户待提现量
        for (uint256 i = 0; i < userStake.unstakeRequests.length; i++) {
        ...
        if (popCount > 0) {
            // 计算剩余解质押请求数量
            uint256 remain = userStake.unstakeRequests.length - popCount;
            if (remain > 0) {
                for (uint256 i = 0; i < remain; i++) {
                    userStake.unstakeRequests[i] = userStake.unstakeRequests[
                        i + popCount
                    ];
                }
            }
        ...
        if (pendingWithdraw > 0) {
            if (stakePool.token == address(0)) {
                ...
                IERC20(stakePool.token).safeTransfer(
                    msg.sender,
                    pendingWithdraw
                );
            }
     ...
     * @param _stakePoolId 质押池ID
     */
    function claim(uint256 _stakePoolId) external whenNotPaused {
        StakePool storage stakePool = stakePools[_stakePoolId];
        UserStake storage userStake = userStakes[_stakePoolId][msg.sender];
        updateStakePoolReward(_stakePoolId);
