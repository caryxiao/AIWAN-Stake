...
contract DeployAwStake is Script {
    function run() external {
        ...
        uint256 rewardStartBlock = vm.envUint("REWARD_START_BLOCK");
        uint256 rewardEndBlock = vm.envUint("REWARD_END_BLOCK");
        uint256 rewardPerBlock = vm.envUint("REWARD_PER_BLOCK");

        vm.startBroadcast(deployerPrivateKey);
        bytes memory initData = abi.encodeCall(
            AwStake.initialize,
            (admin, rewardStartBlock, rewardEndBlock, rewardPerBlock)
        );

        AwStake awStake = AwStake(
            Upgrades.deployUUPSProxy("AwStake.sol:AwStake", initData)
        );
